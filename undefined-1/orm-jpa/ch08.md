...
부모 객체에서 해당 컬랙션에 무엇인가 operation 을 했고 이것을 '영속화' 할 권한을 주는 것(= 해당 컬렉션에 가해진 operation 이 target entity 까지 cascade 되도록 설정) 하는 것이 필요하니까 그런 것 같다. 오히려 REMOVE 만 설정해두니 delete 가 발생하지 않았다.
