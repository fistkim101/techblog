...
* [x] 기존 서비스 다시 분석 - B**(6/26, \~10:00)**
* [ ] GRMC 문서화 전체 재점검**(6/26, 10:00 \~ )**
  * [x] 서비스 한 문장으로 정의. 전달 핵심. 코어 기능 정의, 부가 기능 정의 (텍스트)
  * [x] A,B 분석한 양식 그대로 GRMC 문서화(세부기능 포함)**(6/26, \~12:00)**
  * [x] 정의된 서비스 핵심과 코어, 부가 기능들 기준으로 화면 플로우 다시 설정 **(6/26, \~14:30)**
  * [x] DDD 입각 도메인 모델링 다시 작성 **(6/26, \~17:00)**
  * [ ] 화면 플로우 수정반영(아예 싹 새로 다시 그리던가, 금방함)
    * [ ] 온보딩(프로필)에서 홈으로 이동 화살표 추가
    * [ ] 라운지의 홈, 질문, 인증 다 지우고 라운지 자체에서 필터 제공(일반, 질문, 인증)
    * [ ] 라운지의 홈에서 포스팅 상세 화면 이동
    * [ ] 내가 쓴 글에서 게시글 상세 화살표 지우고 내가 쓴 글에서 탭 두개로 쪼개서 그룹 게시판, 라운지로 분리
  * [ ] 피그마 다시 제작                              \
    \* 현재 피그마 말고 새로운 피그마 작성.\
    \* 세세한 수준까지 디자인 의사 결정 완료. 참고 서비스 스크린 샷 첨부 필수. 어떤 형태로든 확정된 디자인이 정리되어 있어야 한다.                                                                                                                            \
    \* 오직 개발할 때에는 개발에만 신경 쓸 수 있도록 모든 의사결정 완료된 상태로 만들기.
